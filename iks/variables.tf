...
variable "worker_pool_params" {
  type = "map"
  description = "This is a map for the worker specs. Each key in the map has list of values. The length of list should typically be the number of additional worker pools created. Element 'i' in each list corresponds to worker pool 'i'. Eg: if the provided 'worker_pools_num' is 2, and the key 'workers' has value as a list of [\"2\",\"1\"], that would indicate that number of workers in worker pool1 = 2, and number of workers in worker pool2 = 1 "
